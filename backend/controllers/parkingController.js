...
const createParkingTicket = asyncHandler(async (req, res) => {
  const { parking_terminus_id, vehicle_number, vehicle_model, user_id, spot_number, parked_at, payment_method, isPaid, served_by } = req.body;
...
const updateParkingTicket = asyncHandler(async (req, res) => {
  const ticket = await ParkingTicket.findByPk(req.params.id);
  if (ticket) {
    const { parking_terminus_id, vehicle_number, vehicle_model, user_id, spot_number, parked_at, collected_at, payment_method, isPaid, served_by } = req.body;

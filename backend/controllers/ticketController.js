...
const createTicket = asyncHandler(async (req, res) => {
    ...
    await Notification.create({
        ...
        message: `Your ticket for the trip from ${origin} to ${destination} has been created. Ticket Number: ${ticket.ticket_number}.`,
    });
    ...
    const smsMessage = `Your trip from : ${origin} to : ${destination} has been booked.\nTicket number: ${ticket.ticket_number}.\n\nComfortably Safe !`;
    await sendSMSV1({
        to: userPhone, 
        from: '2M Express', 
        sms: smsMessage,
...
const bookTrip = asyncHandler(async (req, res) => {
    ...
    // Fetch the trip along with the route details
    ...
    await Notification.create({
        user_id,
        subject: 'Trip Booked',
        message: `Your trip from ${origin} to ${destination} has been booked pending confirmation. Your ticket number is ${ticket.ticket_number}.\n\nComfortably Safe !`,
    });
    ...
    const smsMessage = `Your trip from : ${origin} to  : ${destination} has been booked pending confirmation. Ticket number: ${ticket.ticket_number}.`;
    await sendSMSV1({
        to: userPhone, 
        from: '2M Express', 
        sms: smsMessage,
...
const updateTicket = asyncHandler(async (req, res) => {
    ...
    await Notification.create({
        ...
        message: `Your ticket for the trip from ${origin} to ${destination} has been updated. Ticket Number: ${updatedTicket.ticket_number}, Seat Number: ${updatedTicket.seat_number}, Status: ${updatedTicket.status}.`,
    });
    ...
    await sendTicketPurchaseEmail(userEmail, ticketDetails);
     ...
     const smsMessage = `Your trip from : ${origin} to : ${destination} has been updated.\nTicket number: ${ticket.ticket_number}.\n\nComfortably Safe !`;
     await sendSMSV1({
         to: userPhone, 
         from: '2M Express', 
         sms: smsMessage,
 ...
 * @access  private
 */
const deleteTicket = asyncHandler(async (ticketId) => {
    try {
        ...
        if (trip.status !== 'embarked') {
            await TripSeat.update(
                { status: 'available' },
                { where: { trip_id: trip.id, seat_number: ticket.seat_number } }
            );
        } else {
            return { message: 'Cannot delete ticket; trip has already embarked.' };
        }
        ...
        console.error(error);
        return { message: 'Error deleting ticket', error: error.message };
    }
});
